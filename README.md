...
2. `HidHide` (HidHide configration client)を起動し `Applications` タグにて本リポジトリよりダウンロードした `resonance-bhop.exe` を登録します。
3. `HidHide` の `Devices` タグで、スターレゾナンスで使用する物理コントローラにチェックを入れます。
4. 下部にある `Enable device hiding` をオンにします。
5. `resonance-bhop.exe` を起動します。
6. BRIDGEの開始 `▶` を押してブリッジを有効化します。
7. 必要なら `オプション` で入力元や閾値を調整します。
8. generic HID / DInput パッドでボタン配置や `L2 / R2` がズレる場合は、`入力キャリブレーション` を実行します。
9. 設定とキャリブレーション結果は自動保存され、次回起動時に復元されます。
10. 慣性ジャンプの補助はASSISTの開始 `▶`ボタン または停止 `■`ボタン で切り替えられます。

## ジャンプ入力について

- ジャンプ入力をデフォルトから変更している場合は、オプションの `ジャンプ入力` にて変更してください。
- 移動入力があるときだけ補助します。
- generic HID / DInput パッドで表示上のボタン名と実機の配置がズレる場合は、先に `入力キャリブレーション` を実行してください。

## 入力キャリブレーションについて

- `入力キャリブレーション` は、generic HID / DInput パッド向けの手動補正機能です。
- `Cross / Circle / Square / Triangle / L1 / R1 / L2 / R2 / Create / Options / L3 / R3` を順番に 1 回ずつ入力して、ボタン配置とトリガー入力を学習させます。
- `BRIDGE` がオフの間は物理入力を読みに行かないため、キャリブレーションは `BRIDGE` をオンにした状態で行ってください。
...
- `resonance-bhop.exe` 自体を `Applications` に入れ忘れると、このツールからも物理コントローラが見えなくなります。
- `XInput` パッドには、非対応です。
- `BRIDGE` をオフにした状態でゲームがまだそのパッドに反応するなら、`HidHide` で隠し切れていない可能性があります。
