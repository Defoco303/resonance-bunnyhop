...
5. 必要なら `オプション` で入力元や閾値を調整します。
6. generic HID / DInput パッドでボタン配置や `L2 / R2` がズレる場合は、`入力キャリブレーション` を実行します。
7. 設定とキャリブレーション結果は自動保存され、次回起動時に復元されます。
8. BRIDGEの開始`▶` を押してブリッジを有効化します。
9. 慣性ジャンプの補助はASSISTの開始`▶` または停止`■` で切り替えられます。

## ジャンプ入力について

- ジャンプ入力は `Circle` 固定です。
- 移動入力があるときだけ補助する設定は常にオンです。
...
- `XInput` パッドは、このツールが入力を読めていても、ゲームから物理パッドを隠せていないと補助が効かないことがあります。
